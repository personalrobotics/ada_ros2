...
async def main(args: argparse.Namespace, pwd: str) -> None:
    ...
    if not args.close:
        print(f"# Starting the calibrate_camera code")
        ...
        print(f"# Terminating the calibrate_camera code")
    print(
        "################################################################################"
    )
    ...
    screen_sessions = {
        ...
        "calibrate": [
            "ros2 run ada_moveit calibrate_camera.py --ros-args "
            f"--params-file {os.path.join(pwd, 'src/ada_ros2/ada_moveit/config/calibrate_camera.yaml')}"
        ],
    }
    close_commands = {}
