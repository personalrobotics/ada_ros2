...
class SimulatedOrientationPublisherNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('reference_frame', 'base_link', reference_frame_descriptor)
        self.declare_parameter('publish_topic', '/articutool/estimated_orientation', publish_topic_descriptor)
        self.declare_parameter('publish_rate', 50.0, publish_rate_descriptor)

        # --- Get Parameters ---
        self.target_link_ = self.get_parameter('target_link').get_parameter_value().string_value
        self.reference_frame_ = self.get_parameter('reference_frame').get_parameter_value().string_value
        self.publish_topic_ = self.get_parameter('publish_topic').get_parameter_value().string_value
        ...
        if not self.target_link_ or not self.reference_frame_:
            self.get_logger().fatal("Parameters 'target_link' and 'reference_frame' must be non-empty strings. Shutting down.")
            # Proper way to exit from init is tricky, rely on context shutdown
            raise ValueError("Missing required parameters")

        if self.publish_rate_ <= 0:
            self.get_logger().fatal("Parameter 'publish_rate' must be positive. Shutting down.")
        ...
        self.tf_listener_ = TransformListener(self.tf_buffer_, self, spin_thread=True) # Use spin_thread=True for automatic background processing
        ...
        self.get_logger().info("Simulated Orientation Publisher node started and listening to TF.")
    ...
    def timer_callback(self):
        ...
        try:
            ...
            lookup_time = Time() # Equivalent to Time(seconds=0, nanoseconds=0) for latest
...
def main(args=None):
    ...
    try:
        ...
        pass
         ...
         if node: node.get_logger().fatal(f"Node initialization failed: {e}")
         else: print(f"Node initialization failed: {e}")
    except Exception as e:
         if node: node.get_logger().error(f"Unhandled exception in main: {e}")
         else: print(f"Unhandled exception in main: {e}")
    finally:
        if node:
            node.destroy_node()
        if rclpy.ok():
